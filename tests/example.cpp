...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/27 20:46:44 by akuburas          #+#    #+#             */
/*   Updated: 2024/11/27 21:28:18 by akuburas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
merge_pair_arrays(std::vector<int>& array, const size_t left, const size_t middle, const size_t right) {
	...
	std::cout << "tmp_left: ";
	print_array(tmp_left.begin(), tmp_left.begin() + left_length);
	std::cout << "tmp_right: ";
	print_array(tmp_right.begin(), tmp_right.begin() + right_length);
...
merge_sort_pairs_recursion(std::vector<int>& array, const size_t left, const size_t right) {
	...
	std::cout << "merge_pair_arrays" << std::endl;
...
merge_sort_pairs(std::vector<int>& array) {
	...
	std::cout << "array.size() - 1 / N_ELEM_IN_PAIR - (is_odd ? 1 : 0): " << (array.size() - 1) / N_ELEM_IN_PAIR - (is_odd ? 1 : 0) << std::endl;
...
main(int argc, char** argv) {
    ...
    std::vector<int> numbers = { 5, 3, 2, 1, 8, 7, 6, 4, 10, 9, 0 };
